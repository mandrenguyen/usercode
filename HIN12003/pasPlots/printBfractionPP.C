...
void printBfractionPP(char *tagger="discr_ssvHighEff", Double_t workingPoint=2, char *taggerName="SSVHE") {
  ...
  TLatex *roots = new TLatex(147,0.0615,"#sqrt{s} = 2.76 TeV");
  roots->Draw();
  ...
  for(Int_t i=1;i<=hBFractionDataLTJP->GetNbinsX();i++) {
    ...
    gSystCL->SetPoint(i-1,hBFractionDataLTJP->GetBinCenter(i),hBFractionDataLTJP->GetBinContent(i));
    gSystMeth->SetPoint(i-1,hBFractionDataLTJP->GetBinCenter(i),hBFractionDataLTJP->GetBinContent(i));
    gSystJES->SetPoint(i-1,hBFractionDataLTJP->GetBinCenter(i),hBFractionDataLTJP->GetBinContent(i));
