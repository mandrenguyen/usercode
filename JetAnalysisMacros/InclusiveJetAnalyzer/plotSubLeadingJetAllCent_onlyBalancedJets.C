...
void plotLeadingJet(int cbin = 0,
		    TString infname = "rootFiles/data.root",
		    TString hydjet = "rootFiles/hydjet.root",
		    TString mix = "rootFiles/mix.root",
		    bool useWeight = true,
...
void plotSubLeadingJetAllCent_onlyBalancedJets() {
  ...
  c1->cd(1);
  plotLeadingJet(2,"rootFiles/data.root","rootFiles/hydjet.root","rootFiles/mix.root",true,false,false);
  gPad->SetLogy();
  ...
  c1->cd(2);
  plotLeadingJet(1,"rootFiles/data.root","rootFiles/hydjet.root","rootFiles/mix.root",true,true,false);
  gPad->SetLogy();
  ...
  c1->cd(3);
  plotLeadingJet(0,"rootFiles/data.root","rootFiles/hydjet.root","rootFiles/mix.root",true,false,true);
  gPad->SetLogy();
  drawText("0~10%",0.10,0.24);
  drawPatch(-0.00007,0.0972,0.0518,0.141);

  TLatex *cms = new TLatex(105,1.8,"CMS Preliminary");
  cms->SetTextFont(63);
  cms->SetTextSize(18);
  cms->Draw();                                                                                                                                        
  //  TLatex *imb = new TLatex(150,0.18,"(p_{T}^{j1}-p_{T}^{j2})/(p_{T}^{j1}+p_{T}^{j2})>0.3");
  TLatex *imb = new TLatex(150,0.27,"AJ<0.3");
  imb->SetTextFont(63);
  imb->SetTextSize(18);
  imb->Draw();  
  
  TLatex *lumi = new TLatex(170,1.8,"#intL dt = 3 #mub^{-1}");
  lumi->SetTextFont(63);
  lumi->SetTextSize(15);
  lumi->Draw(); 

  c1->Print("./fig/dijet_subleadingjet_all_cent_onlyBalancedJets_20101127_v0.gif");
  c1->Print("./fig/dijet_subleadingjet_all_cent_onlyBalancedJets_20101127_v0.eps");
  c1->Print("./fig/dijet_subleadingjet_all_cent_onlyBalancedJets_20101127_v0.pdf");
		    ...
		    bool drawLeg)
{
  ...
  TH1D *h = new TH1D("h","",10,60,220);
  TH1D *hEmbedded = new TH1D("hEmbedded","",10,60,220);
  TH1D *hDataMix = new TH1D("hDataMix","",10,60,220);
  nt->Draw("et2>>h",Form("(%s)",cut.Data())); 
   
  if (useWeight) {
    // use the weight value caluculated by Matt's analysis macro
    ntHydjet->Draw("et2>>hEmbedded",Form("(%s)*weight",cut.Data())); 
    ntMix->Draw("et2>>hDataMix",Form("(%s)*weight",cut.Data())); 
  } else {
    // ignore centrality reweighting
    ntHydjet->Draw("et2>>hEmbedded",Form("(%s)",cut.Data()));
    ntMix->Draw("et2>>hDataMix",Form("(%s)*weight",cut.Data()));  
  }
  ...
  hEmbedded->Draw("hist");
  if(drawXLabel) hEmbedded->SetXTitle("Subleading Jet E_{T} (GeV)");
