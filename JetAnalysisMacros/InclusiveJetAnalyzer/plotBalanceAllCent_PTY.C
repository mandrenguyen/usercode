...
void plotBalanceAllCent_PTY(){
  ...
  c1->Print("./fig/dijet_imbalance_all_cent_20101126_v0_PTY.gif");
  c1->Print("./fig/dijet_imbalance_all_cent_20101126_v0_PTY.eps");
  c1->Print("./fig/dijet_imbalance_all_cent_20101126_v0_PTY.pdf");
		 ...
		 bool drawLeg)
{
  ...
  TH1D *hPythia_trig = new TH1D("hPythia_trig","hPythia_trig",380,120,500);
  TH1D *hDataMix_trig = new TH1D("hDataMix_trig","hDataMix_trig",380,120,500);
   
  nt->Draw("et1>>h_trig",Form("(%s)",trigcut.Data()));
  if(useWeight)ntMix->Draw("et1>>hDataMix_trig",Form("(%s)*weight",trigcut.Data()));
  else ntMix->Draw("et1>>hDataMix_trig",Form("(%s)",trigcut.Data()));
  ntPythia->Draw("et1>>hPythia_trig");

  cout<<" h_trig->Integral() "<<h_trig->Integral()<<endl;
  cout<<" hPythia_trig->Integral() "<<hPythia_trig->Integral()<<endl;
  cout<<" hDataMix_trig->Integral() "<<hDataMix_trig->Integral()<<endl;
  ...
  hPythia->Scale(1./hPythia_trig->Integral(1,380)/hPythia->GetBinWidth(1));
  ...
  if(drawXLabel) hPythia->SetXTitle("A_{J} #equiv (E_{T}^{j1}-E_{T}^{j2})/(E_{T}^{j1}+E_{T}^{j2})");
  ...
  hPythia->SetYTitle("1/N_{leading jet} dN/dA_{J}");
  ...
  hPythia->SetAxisRange(0,4.0,"Y");


  //  hDataMix->Scale(1./hDataMix->Integral(0,20));
  hDataMix->Scale(1./hDataMix_trig->Integral(1,380)/hDataMix->GetBinWidth(1));
...
void drawDum(float min, float max, double drawXLabel){
  ...
  if(drawXLabel) hdum->SetXTitle("(E_{T}^{j1}-E_{T}^{j2})/(E_{T}^{j1}+E_{T}^{j2})");
