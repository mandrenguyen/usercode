...
void plotLeadingPhi_BalanceBin_AllCent(){
  ...
  TLatex *aja= new TLatex(-2.5,0.2,"AJ < 0.3");
  aja->SetTextFont(63);
  aja->SetTextSize(18);
  aja->Draw();
  ...
  TLatex *aj= new TLatex(-2.5,0.2,"AJ > 0.3");
  aj->SetTextFont(63);
  aj->SetTextSize(18);
  aj->Draw();

  c1->Print("./fig/dijet_leadingPhi_balanceBin_all_cent_20101126_v0.gif");
  c1->Print("./fig/dijet_leadingPhi_balanceBin_all_cent_20101126_v0.eps");
  c1->Print("./fig/dijet_leadingPhi_balanceBin_all_cent_20101126_v0.pdf");
		  ...
		  bool balance)
{
  ...
  TH1D *hEmbedded = new TH1D("hEmbedded","",20,-3.14159,3.14159);
  ...
  if (useWeight) {
    ...
    ntHydjet->Draw("phi1>>hEmbedded",Form("(%s)*weight",cut.Data())); 
    ntMix->Draw("phi1>>hDataMix",Form("(%s)*weight",cut.Data())); 
  } else {
    // ignore centrality reweighting
     ntHydjet->Draw("phi1>>hEmbedded",Form("(%s)",cut.Data()));
     ntMix->Draw("phi1>>hDataMix",Form("(%s)*weight",cut.Data()));  
  }
  ...
  if(drawXLabel) hEmbedded->SetXTitle("Leading jet #phi");
