...
void plotRatio(int cbin = 0,
		 TString infname = "data-Ineff.root",
...
void plotJetRecoEfficiencySystematics(){
  ...
  c1->cd(1);
  plotRatio(4,"data-Ineff.root","data.root","data.root",false,false,false);
  ...
  c1->cd(2);
  plotRatio(3,"data-Ineff.root","data.root","data.root",false,false,false);
  ...
  c1->cd(3);
  plotRatio(2,"data-Ineff.root","data.root","data.root",false,true,false);
  ...
  TLatex *lumi = new TLatex(0.15,4.1,"#intL dt = 6.7 #mub^{-1}");
  lumi->SetTextFont(63);
  lumi->SetTextSize(15);
  lumi->Draw(); 
		 ...
		 bool drawLeg)
{
  ...
  if(drawLeg){
    TLegend *t3=new TLegend(0.06,0.67,0.48,0.84); 
    t3->AddEntry(h,"Data with Inefficiency/ Data","pl");
